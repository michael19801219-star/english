...
const QuizView: React.FC<QuizViewProps> = ({ questions, onFinish, onCancel }) => {
  ...
  const startVoiceInput = () => {
    if (recognitionRef.current) {
      ...
      alert("当前浏览器不支持语音识别哦，请手动输入吧。");
    }
  };

  const playTTS = async (text: string, msgIdx: number) => {
    if (playingMsgIdx !== null) return;
    ...
    try {
      ...
      source.onended = () => setPlayingMsgIdx(null);
      source.start(0);
    } catch (err) {
      console.error(err);
      setPlayingMsgIdx(null);
  ...
  return (
    <div className="flex-1 flex flex-col p-6 animate-fadeIn h-screen relative bg-gray-50">
      {isExiting && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md p-6 text-center">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-10 shadow-2xl animate-fadeIn">
            ...
            <p className="text-gray-500 text-sm mb-10">本次训练的进度将无法保存哦。</p>
            <div className="flex flex-col gap-3">
              <button onClick={onCancel} className="w-full py-4.5 bg-red-500 text-white rounded-2xl font-black">确定退出</button>
      ...
      <header className="mb-6 flex-shrink-0">
        <div className="flex justify-between items-center mb-4">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">GAOKAO MASTER</span>
            <span className="text-sm font-black text-indigo-600">第 {currentIndex + 1} 题 / 共 {questions.length} 题</span>
          </div>
          <button onClick={triggerExit} className="px-4 py-2 bg-gray-100 text-gray-500 rounded-full text-xs font-black">退出</button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto pr-1 flex flex-col scroll-smooth" ref={mainScrollRef}>
        <div className="bg-white rounded-[32px] p-7 shadow-sm border border-gray-100 mb-6 relative overflow-hidden flex-shrink-0">
          ...
          <p className="text-lg font-bold leading-relaxed text-gray-800">{currentQuestion.question}</p>
        </div>
        ...
        <div className="space-y-3.5 flex-shrink-0 mb-6">
          {currentQuestion.options.map((option, idx) => {
            ...
            if (showFeedback) {
              if (idx === currentQuestion.answerIndex) style = "border-green-500 bg-green-50 text-green-700 font-bold ring-2 ring-green-500/10";
            ...
            return (
              <button key={idx} disabled={showFeedback} onClick={() => setSelectedOption(idx)} className={`w-full p-4 rounded-[20px] text-left transition-all border-2 relative flex items-center ${style}`}>
          ...
          <div className="flex flex-col gap-6 animate-fadeIn pb-8">
            <div className="p-6 bg-white rounded-[28px] border border-indigo-50 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-lg ${isCorrect ? 'bg-green-100' : 'bg-red-100'}`}>{isCorrect ? '✨' : '📖'}</div>
                <h4 className={`font-black tracking-tight ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>{isCorrect ? '回答正确！' : '错点突破'}</h4>
              </div>
              <div className="bg-gray-50 p-4 rounded-2xl">
                <p className="text-[14px] text-gray-600 leading-relaxed whitespace-pre-wrap font-medium">{currentQuestion.explanation}</p>
              </div>
            </div>

            {/* AI 聊天区域 */}
            <div className="p-6 bg-indigo-50 rounded-[32px] border border-indigo-100 flex flex-col">
              ...
              <div className="flex flex-col gap-3 mb-6 max-h-[400px] overflow-y-auto">
                    ...
                    <div className={`max-w-[85%] p-4 rounded-2xl text-sm font-medium relative ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-700 rounded-bl-none border border-indigo-100'}`}>
                      ...
                      {msg.role === 'model' && (
                        <button 
                          ...
                          className={`absolute -right-2 -bottom-2 w-7 h-7 bg-white border border-indigo-100 rounded-full flex items-center justify-center shadow-sm active:scale-90 ${playingMsgIdx === idx ? 'animate-pulse' : ''}`}
                        >
                          {playingMsgIdx === idx ? (
                ...
                {isAsking && <div className="flex justify-start"><div className="bg-white p-4 rounded-2xl rounded-bl-none border border-indigo-100 flex gap-1.5 animate-pulse"><div className="w-1.5 h-1.5 bg-indigo-300 rounded-full"></div><div className="w-1.5 h-1.5 bg-indigo-300 rounded-full"></div><div className="w-1.5 h-1.5 bg-indigo-300 rounded-full"></div></div></div>}
                <div ref={chatEndRef} />
              </div>
              ...
              <div className="relative flex items-center gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder={isRecognizing ? "正在聆听..." : "针对考点追问..."}
                    className={`w-full py-4 pl-5 pr-12 bg-white rounded-2xl border-none text-sm font-black shadow-sm transition-all ${isRecognizing ? 'ring-2 ring-indigo-400' : ''}`}
                    disabled={isAsking}
                  />
                  <button
                    onClick={startVoiceInput}
                    className={`absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center transition-all ${isRecognizing ? 'bg-red-500 text-white animate-bounce' : 'text-gray-400 hover:text-indigo-600'}`}
                  >
                ...
                <button
                  ...
                  className={`p-4 rounded-2xl shadow-md transition-all ${!followUpQuery.trim() || isAsking ? 'bg-gray-200 text-gray-400' : 'bg-indigo-600 text-white active:scale-90'}`}
                >
      ...
      <footer className="py-6 flex-shrink-0 safe-area-bottom">
        {!showFeedback ? (
          <button disabled={selectedOption === null} onClick={handleConfirm} className={`w-full py-4.5 rounded-[24px] font-black text-lg shadow-xl ${selectedOption === null ? 'bg-gray-200 text-gray-400' : 'bg-indigo-600 text-white shadow-indigo-200'}`}>确认提交</button>
        ) : (
